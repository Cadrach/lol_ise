...
import fileSaverModule from 'angular-file-saver';

export default 'appLolIse.set';
angular.module('appLolIse.set', [fileUploadModule, uibModule, fileSaverModule])
...
.directive('setDownloader', ['$window', '$uibModal', '$timeout', 'FileSaver', function($window, $uibModal, $timeout, FileSaver) {
    return {
        link: function(scope, elmt){
            /**
             * **************************************************************************************
            ...
            scope.download = function(){
                ...
                zip.generateAsync({type:"blob"}).then(blob => {FileSaver.saveAs(blob, filename, true);});
            }
